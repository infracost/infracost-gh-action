...
absolute_percent_diff=$(bc <<<"scale=4; $master_monthly_cost / $pull_request_monthly_cost * 100 - 100" | tr -d -)

if (( $(echo "$absolute_percent_diff >= $percentage_threshold" | bc -l) )); then
